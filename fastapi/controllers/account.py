...
from models.account import JWTModel, UserModel
...
class Account():
    ...
    def login(self, email: str, password: str, ip: IPvAnyAddress = None):
        ...
        with MongoClient(MONGO_AUTH) as c:
            ...
            if user is None:
                raise HTTPException(
                    404, "Utente non registrato. Procedi prima con la registrazione del tuo account.")
            user = UserModel(**user)

            # verifica la password
            if not bcrypt.checkpw(password.encode(), user.hashed_password.encode()):
                raise HTTPException(500,
                                    "password non corretta per questo account.")

            # crea il token filtrando i dati sensibili
            token: str = JWT.create(user.dict(exclude={
                'id',
                'hashed_password',
                'registration_date'
            }))
            ...
            access = c.users.accesses.insert_one({
                "uid": str(jwt.uid),
                "jti": str(jwt.jti),
                "ip": ip,
    ...
    def register(self, email: str, password: str, ip: IPvAnyAddress = None) -> str:
        ...
        if(self.exists(email_hash)):
            raise HTTPException(400,
                                "un utente con questa email esiste gia' nel database")
        salt = bcrypt.gensalt()
            ...
            with c.start_session() as s:
                with s.start_transaction() as t:
                    ...
                    while True:
                        ...
                        if c.users.operations.find_one({"key": activation_key}) is None:
                            break

                    # inserisce il nuovo utente
                    id = c.users.users.insert_one(
                        {
                            "uid": str(uid),
                            "email": email,
                            "active": False,
                    ...
                    if id is None:
                        raise HTTPException(
                            500, str("errore inserimento nuovo utente"))
    ...
    def exists(self, email_hash: str) -> bool:
        with MongoClient(MONGO_AUTH) as c:
            emails = [
                ...
                for e
                in c.users.users.find({}, {"email": 1, "_id": 0})
            ]
